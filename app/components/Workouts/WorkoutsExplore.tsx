...
import { deleteWorkout as deleteWorkoutApi, editWorkout as editWorkoutApi } from '../../Utils/utils'
...
const WorkoutsExplore = ({email, initialWorkouts}: Props) => {
    ...
    const editWorkout = async (id: number, data: any) => {
        await editWorkoutApi(id, data)
        const updatedWorkouts = workouts.map(w => w.id === id ? { ...w, ...data } : w)
        setWorkouts(updatedWorkouts)
        setWorkoutsDisplay(prev => prev.map(w => w.id === id ? { ...w, ...data } : w))
    }
    ...
    return (
        <div className='flex-1 min-w-0'>
            ...
            <div className='relative mb-4'>
                ...
                <input type="text" placeholder={t('workouts.searchWorkouts')} value={search} onChange={handleSearch} 
            ...
            <div className='max-h-[500px] overflow-auto space-y-3 pr-1'>
                ...
                <div className='flex flex-col items-center justify-center py-12 text-center'>
                    ...
                    <p className='text-sm text-slate-500'>{t('workouts.noWorkouts')}</p>
                    <p className='text-xs text-slate-400 mt-1'>{t('workouts.addFirst')}</p>
                </div>}
            </div>
        </div>
    )
