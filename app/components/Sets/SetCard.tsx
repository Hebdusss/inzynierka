  ...
  return (
    <div className='card-glass p-5 mr-4 mb-4 w-80 group'>
        <div className='flex items-center justify-between mb-4'>
          ...
          {isPublic && (
            <span className='text-xs font-medium text-slate-400 border border-slate-200 rounded-full px-2.5 py-0.5'>{t('set.public')}</span>
          )}
        </div>

        <div className='grid grid-cols-3 gap-2 mb-4'>
          <div className='bg-slate-50 rounded-lg px-3 py-2 text-center'>
              <p className='text-xs text-slate-400'>{t('set.burned')}</p>
              <p className='text-sm font-bold text-slate-700'>{caloriesBurned}</p>
          </div>
          <div className='bg-slate-50 rounded-lg px-3 py-2 text-center'>
              <p className='text-xs text-slate-400'>{t('set.consumed')}</p>
              <p className='text-sm font-bold text-slate-700'>{caloriesConsumed}</p>
          </div>
          <div className='bg-slate-50 rounded-lg px-3 py-2 text-center'>
              <p className='text-xs text-slate-400'>{t('set.time')}</p>
        ...
        <div className='flex gap-2 mb-4'>
          <details className="dropdown flex-1">
              <summary className="w-full px-3 py-2 text-xs font-medium bg-slate-100 text-slate-600 rounded-lg cursor-pointer hover:bg-slate-200 transition-colors text-center">
                {t('set.workouts')} ({workouts.length})
              </summary>
        ...
        <button className="btn-danger w-full opacity-0 group-hover:opacity-100 transition-opacity text-center" onClick={() => onClick(id)}>{t('set.delete')}</button>
    </div>
  )
}
